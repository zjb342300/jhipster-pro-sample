...
import com.mycompany.myapp.service.CommonConditionQueryService;
import com.mycompany.myapp.service.CommonConditionService;
 ...
 * 管理实体{@link com.mycompany.myapp.domain.CommonCondition}的REST Controller。
 */
@RestController
@RequestMapping("/api")
public class CommonConditionResource {
    ...
    public ResponseEntity<List<CommonConditionDTO>> getAllCommonConditions(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = commonConditionQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = commonConditionQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = commonConditionQueryService.findBySpecification(specification, pageable);
            } else {
                page = commonConditionQueryService.findByCriteria(criteria, pageable);
            }
        }
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<CommonConditionDTO> page = commonConditionService.findAll(Pageable.unpaged());
