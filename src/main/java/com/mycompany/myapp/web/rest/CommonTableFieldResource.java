...
import com.mycompany.myapp.service.CommonTableFieldQueryService;
import com.mycompany.myapp.service.CommonTableFieldService;
 ...
 * 管理实体{@link com.mycompany.myapp.domain.CommonTableField}的REST Controller。
 */
@RestController
@RequestMapping("/api")
    ...
    public ResponseEntity<List<CommonTableFieldDTO>> getAllCommonTableFields(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = commonTableFieldQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = commonTableFieldQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = commonTableFieldQueryService.findBySpecification(specification, pageable);
            } else {
