...
public class SmsConfigResource {
    ...
    public ResponseEntity<List<SmsConfigDTO>> getAllSmsConfigs(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = smsConfigQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = smsConfigQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = smsConfigQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<SmsConfigDTO> page = smsConfigService.findAll(Pageable.unpaged());
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), SmsConfigDTO.class, page.getContent());
