...
public class ResourceCategoryResource {
    ...
    public ResponseEntity<List<ResourceCategoryDTO>> getAllResourceCategories(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = resourceCategoryQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = resourceCategoryQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = resourceCategoryQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<List<ResourceCategoryDTO>> getAllResourceCategoriesofTree(Pageable pageable) {
        ...
        Page<ResourceCategoryDTO> page = resourceCategoryService.findAllTop(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<ResourceCategoryDTO> page = resourceCategoryService.findAll(Pageable.unpaged());
