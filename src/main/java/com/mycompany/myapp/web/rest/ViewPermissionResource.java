...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.ViewPermission}.
 */
@RestController
@RequestMapping("/api")
    ...
    public ResponseEntity<List<ViewPermissionDTO>> getAllViewPermissions(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = viewPermissionQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = viewPermissionQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = viewPermissionQueryService.findBySpecification(specification, pageable);
            } else {
