...
public class OssConfigResource {
    ...
    public ResponseEntity<List<OssConfigDTO>> getAllOssConfigs(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = ossConfigQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = ossConfigQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = ossConfigQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<OssConfigDTO> page = ossConfigService.findAll(Pageable.unpaged());
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), OssConfigDTO.class, page.getContent());
