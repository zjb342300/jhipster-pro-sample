...
import com.mycompany.myapp.service.GpsInfoQueryService;
import com.mycompany.myapp.service.GpsInfoService;
 ...
 * 管理实体{@link com.mycompany.myapp.domain.GpsInfo}的REST Controller。
 */
@RestController
@RequestMapping("/api")
public class GpsInfoResource {
    ...
    public ResponseEntity<List<GpsInfoDTO>> getAllGpsInfos(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = gpsInfoQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = gpsInfoQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = gpsInfoQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        ...
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), GpsInfoDTO.class, page.getContent());
