...
public class PositionResource {
    ...
    public ResponseEntity<List<PositionDTO>> getAllPositions(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = positionQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = positionQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = positionQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<PositionDTO> page = positionService.findAll(Pageable.unpaged());
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), PositionDTO.class, page.getContent());
