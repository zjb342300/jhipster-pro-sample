...
import com.mycompany.myapp.service.StatisticsApiQueryService;
import com.mycompany.myapp.service.StatisticsApiService;
...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.StatisticsApi}.
 */
@RestController
@RequestMapping("/api")
public class StatisticsApiResource {
    ...
    public ResponseEntity<List<StatisticsApiDTO>> getAllStatisticsApis(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = statisticsApiQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = statisticsApiQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = statisticsApiQueryService.findBySpecification(specification, pageable);
            } else {
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<StatisticsApiDTO> page = statisticsApiService.findAll(Pageable.unpaged());
