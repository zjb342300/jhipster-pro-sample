...
import com.mycompany.myapp.service.CommonConditionItemQueryService;
import com.mycompany.myapp.service.CommonConditionItemService;
 ...
 * 管理实体{@link com.mycompany.myapp.domain.CommonConditionItem}的REST Controller。
 */
@RestController
@RequestMapping("/api")
public class CommonConditionItemResource {
    ...
    public ResponseEntity<List<CommonConditionItemDTO>> getAllCommonConditionItems(
        ...
        Page<CommonConditionItemDTO> page;
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = commonConditionItemQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = commonConditionItemQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = commonConditionItemQueryService.findBySpecification(specification, pageable);
            } else {
                page = commonConditionItemQueryService.findByCriteria(criteria, pageable);
            }
        }
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<CommonConditionItemDTO> page = commonConditionItemService.findAll(Pageable.unpaged());
