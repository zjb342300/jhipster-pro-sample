...
public class DepartmentResource {
    ...
    public ResponseEntity<List<DepartmentDTO>> getAllDepartments(
        ...
        if (listModelName != null) {
            if (commonQueryId != null) {
                ...
                page = departmentQueryService.selectByCustomEntity(listModelName, criteria, null, specification, pageable);
            } else {
                page = departmentQueryService.selectByCustomEntity(listModelName, criteria, null, null, pageable);
            }
        } else {
            if (commonQueryId != null) {
                Specification specification = commonConditionQueryService.createSpecification(commonQueryId);
                page = departmentQueryService.findBySpecification(specification, pageable);
            } else {
                page = departmentQueryService.findByCriteria(criteria, pageable);
            }
        }
    ...
    public ResponseEntity<List<DepartmentDTO>> getAllDepartmentsofTree(Pageable pageable) {
        ...
        Page<DepartmentDTO> page = departmentService.findAllTop(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
    ...
    public ResponseEntity<Void> exportToExcel() throws IOException {
        Page<DepartmentDTO> page = departmentService.findAll(Pageable.unpaged());
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("计算机一班学生", "学生"), DepartmentDTO.class, page.getContent());
