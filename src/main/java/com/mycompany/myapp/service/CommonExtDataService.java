...
public class CommonExtDataService {
    ...
    public Owner getExtData(Owner source) {
        ...
        commonExtDataRepository
            .findByOwnerIdAndOwnerType(source.getId(), source.getOwnerEntityName())
            .forEach(
                commonExtData -> {
                    switch (commonExtData.getCommonFieldType()) {
                        case "CommonBoolean":
                            CommonBoolean commonBoolean = (CommonBoolean) commonExtData.getValue();
                            extData.put(commonExtData.getFieldName(), commonBoolean.getValue());
                            break;
                        case "CommonString":
        ...
        extData.forEach(
            (key, value) -> {
                ...
                if (finalMultiModel != null) {
                    ...
                    field = commonTableFieldRepository.findOneByCommonTableEntityNameAndEntityFieldName(source.getOwnerEntityName(), key);
                }
                if (field != null) {
    ...
    private void saveString(Owner source, String key, Object value) {
        CommonExtData commonExtData = commonExtDataRepository
            .findByOwnerIdAndOwnerTypeAndFieldName(source.getId(), source.getOwnerEntityName(), key)
            .orElse(new CommonExtData());
    ...
    private void saveBoolean(Owner source, String key, Object value) {
        ...
        if (commonExtData.getId() != null) {
            ...
            if (commonBoolean == null) {
                ...
                update = true;
            } else if (!commonBoolean.getValue().equals(value)) {
                update = true;
    ...
    public Page<CommonExtDataDTO> findAll(Pageable pageable) {
        ...
        return commonExtDataRepository.findAll(pageable).map(commonExtDataMapper::toDto);
    }
    ...
    public Optional<CommonExtDataDTO> findOne(Long id) {
        ...
        return commonExtDataRepository.findById(id).map(commonExtDataMapper::toDto);
    }
