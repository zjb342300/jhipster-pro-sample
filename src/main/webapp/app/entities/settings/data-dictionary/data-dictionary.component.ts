...
@Component
export default class DataDictionaryComponent extends Vue {
  ...
  public prepareRemove(instance: IDataDictionary): void {
  ...
  public removeById(removeId: number): void {
    this.dataDictionaryService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.settingsDataDictionary.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: IDataDictionary): void {
    if (this.showInOther || this.editInModal) {
      this.dataDictionaryId = row.id;
  ...
  public addChild(parent: IDataDictionary): void {
    if (this.showInOther || this.editInModal) {
      this.updateModalVisible = true;
      this.parentId = parent.id;
    } else {
      // @ts-ignore
      this.$router.push({ name: 'settings-data-dictionary-new', params: { parentId: parent.id }, append: true });
    }
  }
  ...
  public initRelationships(): void {
    ...
    forkJoin([this.dataDictionaryService().tree()]).subscribe(
      ([dataDictionaries]) => {
        ...
        const listOfFilterparent = dataDictionaries.data.slice(0, dataDictionaries.data.length > 8 ? 7 : dataDictionaries.data.length - 1);
