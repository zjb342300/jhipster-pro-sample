...
@Component
export default class PositionCompactComponent extends Vue {
  ...
  public positionCommonTableData: ICommonTable = null; // 实体模型数据
  ...
  public prepareRemove(instance: IPosition): void {
  ...
  public removeById(removeId: number): void {
    this.positionService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.settingsPosition.deleted', { param: this.removeId }).toString();
