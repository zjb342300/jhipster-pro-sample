...
@Component
export default class GpsInfoComponent extends Vue {
  ...
  public prepareRemove(instance: IGpsInfo): void {
  ...
  public removeById(removeId: number): void {
    this.gpsInfoService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.settingsGpsInfo.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: IGpsInfo): void {
    if (this.showInOther || this.editInModal) {
      this.gpsInfoId = row.id;
