...
@Component
export default class SmsConfigComponent extends Vue {
  ...
  public prepareRemove(instance: ISmsConfig): void {
  ...
  public removeById(removeId: number): void {
    this.smsConfigService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.filesSmsConfig.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ISmsConfig): void {
    if (this.showInOther || this.editInModal) {
      this.smsConfigId = row.id;
