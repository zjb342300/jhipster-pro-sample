...
@Component
export default class OssConfigComponent extends Vue {
  ...
  public prepareRemove(instance: IOssConfig): void {
  ...
  public removeById(removeId: number): void {
    this.ossConfigService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.filesOssConfig.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: IOssConfig): void {
    if (this.showInOther || this.editInModal) {
      this.ossConfigId = row.id;
