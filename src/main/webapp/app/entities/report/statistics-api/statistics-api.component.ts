...
@Component
export default class StatisticsApiComponent extends Vue {
  ...
  public totalItems = 0;
  public omitFields = ['id', 'apiBody', 'result'];
  public statisticsApis: IStatisticsApi[] = [];
  public mapOfFilter: { [key: string]: any } = {
    commonTable: { list: [], value: [], type: 'many-to-one' },
    creator: { list: [], value: [], type: 'many-to-one' },
    modifier: { list: [], value: [], type: 'many-to-one' },
  };
  ...
  public prepareRemove(instance: IStatisticsApi): void {
  ...
  public removeById(removeId: number): void {
    this.statisticsApiService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.reportStatisticsApi.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: IStatisticsApi): void {
    if (this.showInOther || this.editInModal) {
      this.statisticsApiId = row.id;
  ...
  public initRelationships(): void {
    ...
    forkJoin([this.commonTableService().retrieve(), this.userService().retrieve()]).subscribe(
      ([commonTables, users]) => {
        ...
        const listOfFiltermodifier = users.data.slice(0, users.data.length > 8 ? 7 : users.data.length - 1);
        this.mapOfFilter.modifier = { list: listOfFiltermodifier, value: [], type: 'many-to-one' };
