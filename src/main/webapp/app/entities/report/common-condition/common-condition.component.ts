...
@Component
export default class CommonConditionComponent extends Vue {
  ...
  public removeById(removeId: number): void {
    this.commonConditionService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.reportCommonCondition.deleted', { param: this.removeId }).toString();
  ...
  public editEntity(row: ICommonCondition): void {
    if (this.showInOther || this.editInModal) {
      this.commonConditionId = row.id;
