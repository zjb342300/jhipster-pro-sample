...
@Component
export default class CommonTableCompactComponent extends Vue {
  ...
  public removeById(removeId: number): void {
    this.commonTableService()
      ...
      .subscribe((res: AxiosResponse) => {
        const message = this.$t('jhipsterSampleApplicationApp.modelConfigCommonTable.deleted', { param: this.removeId }).toString();
  ...
  public initRelationships(): void {
    this.loading = true;
    forkJoin([this.commonTableService().retrieve(), this.userService().retrieve(), this.businessTypeService().retrieve()]).subscribe(
      ([commonTables, users, businessTypes]) => {
