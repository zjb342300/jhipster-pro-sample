...
export default class CommonTableUpdate extends Vue {
  ...
  public save(): void {
    ...
    this.updateForm
      .getData()
      .then(values => {
        this.commonTable = Object.assign(this.commonTable, values);
        if (this.commonTable.id) {
          this.commonTableService()
            .update(this.commonTable)
            .subscribe(param => {
              this.isSaving = false;
              const message = this.$t('jhipsterSampleApplicationApp.modelConfigCommonTable.updated', { param: param.data.id }).toString();
          ...
          this.commonTableService()
            ...
            .subscribe(param => {
              this.isSaving = false;
              const message = this.$t('jhipsterSampleApplicationApp.modelConfigCommonTable.created', { param: param.data.id }).toString();
